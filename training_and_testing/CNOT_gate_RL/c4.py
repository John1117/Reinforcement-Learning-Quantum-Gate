...
from pack.env import X_gate

s = 0.01
k = 1.5 ** 0.5

target_gate = np.kron(X_gate, X_gate) # CNOT
max_gate_time = 4
num_steps = 16
Z1_const = 0.
...
max_J_amp = 4.0
fix_J = True
fix_J_value = 0. # 1 / max_gate_time * 2
train_ideal_weight = 1.0
train_noise_weight = 1.0
dtype = np.float64

init_learning_rate = 3e-4
actor_network_config = (128, 128)
value_network_config = (128, 128)
init_action_stddev = np.array(1e-1, dtype)
...
main_path = '/Users/john/personal/myself/projects/MLQuantGate/cnot_record/ideal_x1x2'
model_name = 'x1'
...
with tf.device('/cpu'):
    ...
    #old_main_path = '/Users/john/personal/myself/projects/MLQuantGate/cnot_record/noisy_z1z2'
    #agent = load_network(agent, old_main_path + '/c17/network/17422', use_stddev_proj_network=False)
